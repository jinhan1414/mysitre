...
function tx() {
    $.ajax({
        type: "POST", url: "myPhoto", data: {tx:myFrame.window.tx,key:myFrame.window.key}, success: function (msg) {
            if (msg != "n") {
                $(".tou").attr('src', "static/img/tx/" + msg + ".png");
...
function EmailAutoComplete(options) {
    this.config = {
        ...
        hoverBg: 'hoverBg',
        inputValColor: 'black',
        mailArr: ["@qq.com", "@163.com", "@126.com", "@gmail.com", "@hotmail.com", "@yahoo.com", "@yahoo.com.cn", "@live.com", "@sohu.com", "@sina.com"],
        isSelectHide: true,
        callback: null
    };
    this.cache = {
...
EmailAutoComplete.prototype = {
    ...
    init: function(options) {
        ...
        $(document).click(function(e) {
            ...
            if (!$(target).hasClass(tagCls)) {
                $('.auto-tip') && $('.auto-tip').each(function(index, item) { ! $(item).hasClass('hidden') && $(item).addClass('hidden')
                })
            }
        });
    },
    _renderHTML: function(cfg) {
        ...
        if (curIndex > -1) {
            ...
            $('.auto-tip li', cfg.parentNode).each(function(index, item) {
                $('.output-num', item).html(curVal); ! $('.output-num', item).hasClass(_config.inputValColor) && $('.output-num', item).addClass(_config.inputValColor);
    ...
    _accurateMate: function(cfg) {
        ...
        if (/@/.test(curVal)) {
            ...
            if (newArrs.length > 0) {
                ...
                $('.auto-tip li', cfg.parentNode).each(function(index, item) {
                    $('.output-num', item).html(prefix); ! $('.output-num', item).hasClass(_config.inputValColor) && $('.output-num', item).addClass(_config.inputValColor);
                    var emVal = $.trim($('.em', item).attr('data-html'));
                    $(item).attr('data-html', '');
                    $(item).attr({
    ...
    _itemHover: function(parentNode) {
        var self = this,
            _config = self.config,
            _cache = self.cache;
        $('.auto-tip li', parentNode).hover(function(index, item) { ! $(this).hasClass(_config.hoverBg) && $(this).addClass(_config.hoverBg)
            },
            function() {
    ...
    _executeClick: function(parentNode) {
        ...
        $('.auto-tip' + ' li', parentNode).bind('click',
            function(e) {
