...
class Admin extends AdminAuth
{
    ...
    public $obj = array();//对象数组
    //模块基本信息
    private $data = array(
        'module_name' => '管理员',
        'module_url'  => '/admin/admin/',
        'module_slug' => 'admin',
    ...
    public function get_user_list(){
        ...
        $this->data["module_name"] = "用户";
        $list = $this->obj["customer"]->customer_list();
    ...
    {
        parent::__construct();
        $this->obj["customer"] = new Customer();
        $this->obj["nav"] = new Navigation();
    }


    public function get_top_nav_list(){
        $nav = $this->obj["nav"]->get_nav('top_nav');
        $data = array("data"=>$this->data,"list"=>$nav);
        return $this->fetch("nav_list",$data);
    }

    public function get_guide_nav_list(){

        $nav = $this->obj["nav"]->get_nav('guide_nav');
        $data = array("data"=>$this->data,"list"=>$nav);
        return $this->fetch("nav_list",$data);
    ...
    {
        $list =  Administrator::where('status','>=','0')->order('id', 'ASC')->paginate(10,true);
    ...
    public function add()
    {
        ...
        if ($id = $user->validate(true)->insertGetId($data)) {
            return $this->success('用户添加成功',"/admin/list_user");
        } else {
